...
class Glasbey:
    ...
    def generate_palette(self, size):
        ...
        if self.overwrite_base_palette:
            self.save_palette(
                palette=self.palette, path=self.base_palette, format="byte", overwrite=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-black", action="store_true", help="avoid black and similar colors")
    parser.add_argument(
        "--lightness-range",
    ...
    parser.add_argument(
        "--chroma-range", type=ast.literal_eval, help="set min and max for chroma (e.g. 10,100)",
    )
    parser.add_argument(
        "--hue-range", type=ast.literal_eval, help="set start and end for hue (e.g. 315,45)"
    )
    ...
    parser.add_argument("output", type=argparse.FileType("w"), help="output palette filename")
    ...
    gb = Glasbey(
        base_palette=args.base_palette.name,
